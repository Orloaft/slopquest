...
// Editor-authored teleport pads (content/portals.editor.yaml). Purely editor-
// owned (no hand-authored base — the legacy char-based portals live in shared.ts).
// Each pad: step on (floor,x,y) → travel to (toFloor,toX,toY). Optional file.
interface RawPortalOverlay {
  portals?: Array<{ floor?: number; x?: number; y?: number; toFloor?: number; toX?: number; toY?: number }>;
}
const portalsOverlay = loadOptional<RawPortalOverlay>("portals.editor.yaml") ?? {};
const PORTALS = (portalsOverlay.portals ?? []).map((p) => ({
  floor: p.floor!, x: p.x!, y: p.y!, toFloor: p.toFloor!, toX: p.toX!, toY: p.toY!
}));
...
for (const p of PORTALS) {
  for (const k of ["floor", "x", "y", "toFloor", "toX", "toY"] as const) {
    if (typeof p[k] !== "number") fail("portals.editor.yaml", `portal missing numeric "${k}" (${JSON.stringify(p)})`);
  }
}
