// Verification: reconstruct the GENERATED Northwood stage exactly as the engine
// reads it (base-layer "forest:<idx>" refs blitted from the packed forest.png +
// objects[] drawn as bottom-center sprites, y-sorted) and write a preview PNG.
// If this matches artifacts/northwood-trees.png (the locked compositor render),
// the bridge is faithful.
...
const ts = 32;
const dir = nodePath.join(repoRoot, "assetsources/asset-forge/exports/northwood");
const stage = JSON.parse(readFileSync(nodePath.join(dir, "northwood.stage.json"), "utf8"));
const sheet = PNG.sync.read(readFileSync(nodePath.join(dir, "forest.png")));
...
function blitFromSheet(idx: number, dx: number, dy: number) {
  ...
  for (let y = 0; y < ts; y++) for (let x = 0; x < ts; x++) {
    ...
    const a = sheet.data[si + 3]; if (a === 0) continue;
    out.data[di] = sheet.data[si]; out.data[di + 1] = sheet.data[si + 1]; out.data[di + 2] = sheet.data[si + 2]; out.data[di + 3] = 255;
  }
}
// tile layers in order (base then fringe), exactly as the engine composites them
...
// objects (bottom-center, y-sorted) — mirrors placeMapSprite setOrigin(0.5,1)+setDisplaySize
const sprites = new Map<string, PNG>();
const loadSprite = (key: string) => {
  if (!sprites.has(key)) {
    const id = key.replace("spriteNw", "");
    sprites.set(key, PNG.sync.read(readFileSync(nodePath.join(repoRoot, `public/sprites/nw/obj_${id}.png`))));
  }
  return sprites.get(key)!;
};
const objs = [...stage.objects].sort((a, b) => a.y - b.y);
for (const o of objs) {
  const p = loadSprite(o.key);
...
const outPath = nodePath.join(repoRoot, "artifacts/northwood-from-stage.png");
writeFileSync(outPath, PNG.sync.write(out));
console.log(`preview from generated stage -> ${outPath} (${W}x${H}); ${objs.length} objects, ${cols}-col tileset`);
