#!/usr/bin/env python3
"""Build a strict equal-cell actor runtime sheet for the tactics prototype."""
...
ROLES = [
    Role("iron-guard", "Iron Guard", 86, 76, 84, 1.02, 1.08, 1.0),
    Role("verdant-ranger", "Verdant Ranger", 90, 72, 84, 1.03, 1.10, 1.04),
    Role("radiant-acolyte", "Radiant Acolyte", 82, 72, 84, 1.0, 1.08, 0.98),
    Role("grave-skitter", "Grave Skitter", 92, 56, 77, 1.14, 1.24, 0.94),
    Role("stone-brute", "Stone Brute", 92, 76, 86, 1.06, 1.16, 0.96),
    Role("grave-archer", "Grave Archer", 94, 70, 84, 1.16, 1.26, 0.95),
]
...
def quantize_rgba(image: Image.Image, colors: int = 15) -> Image.Image:
    alpha = image.getchannel("A")
...
def add_outline(image: Image.Image, role: Role) -> Image.Image:
    ...
    outline = Image.new("RGBA", image.size, (9, 10, 10, 0))
    outline.putalpha(outline_alpha)
    outlined = Image.alpha_composite(outline, image)

    if role.id in {"grave-skitter", "grave-archer"}:
        rim = Image.new("RGBA", image.size, (237, 203, 152, 0))
        rim_alpha = alpha.filter(ImageFilter.MaxFilter(3))
        rim_alpha = Image.eval(rim_alpha, lambda value: 86 if value > 0 else 0)
...
def main() -> int:
    ...
    manifest = {
        "version": "battle-stage-actor-slicing-v2",
