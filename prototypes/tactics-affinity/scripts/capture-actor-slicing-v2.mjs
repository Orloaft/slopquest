...
const artifactDir = path.join(prototypeRoot, "artifacts/battle-stage-actor-slicing-v2");
const mediaDir = path.join(process.env.HOME, ".openclaw/media/tib-gathering/battle-stage-actor-slicing-v2");

const shots = [
  { name: "battle-stage-actor-slicing-desktop.png", viewport: { width: 1440, height: 900 }, route: "/" },
  { name: "battle-stage-actor-slicing-mobile.png", viewport: { width: 760, height: 1280 }, route: "/" },
  { name: "battle-stage-actor-slicing-grayscale.png", viewport: { width: 1440, height: 900 }, route: "/?gray=1" }
];
...
async function assertRuntimeSheet(page) {
  const loaded = await page.evaluate(() => {
    ...
    return {
      ...
      versionCount: actors.filter((node) => node.dataset.actorSheet === "actor-slicing-v2").length,
      background
    };
  });
  ...
  if (loaded.versionCount !== loaded.actorCount) {
    throw new Error(`Expected all generated actors to use actor-slicing-v2, found ${loaded.versionCount}/${loaded.actorCount}`);
  }
  if (!loaded.background.includes("/assets/generated/actor-slicing-v2/generated-low-res-actor-poses.png")) {
...
async function main() {
  ...
  try {
    ...
    const report = [
      "# Battle Stage Actor Slicing V2 Runtime Proof",
      "",
      `- Dev server: ${baseUrl}`,
      `- Port: ${port}`,
      "- Runtime actor sheet: /assets/generated/actor-slicing-v2/generated-low-res-actor-poses.png",
      "- Runtime check: every .generated-actor node has data-actor-sheet=actor-slicing-v2 and computed background-image points at the V2 equal-cell sheet.",
      "- Screenshots:",
    ...
    console.log(`captured battle-stage-actor-slicing-v2 screenshots on port ${port}`);
  } finally {
